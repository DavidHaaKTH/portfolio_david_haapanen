...
- **Del 2: Hamiltonian Monte Carlo (HMC)**
  ...
  - Jämfördes mot vanlig Metropolis–Hastings med en bivariate randomwalk proposal, med parametrar: $` \sigma = 2, \Sigma = \text{diag}(5, 0.5) `$. Posterior antogs vara $``f(\theta\mid y)``$ där $``y_i\sim N(\|\theta\|^2,\sigma^2)``$.
